...
static uint64_t control_time_ns(size_t cycle) {
  uint64_t base_ns =
      (UINT64_C(100000) + cycle * UINT64_C(5000)) * UINT64_C(1000);

  if (cycle <= CYCLE_NAV_RATE_RECOVERY) {
    return base_ns;
  }
  /* From CYCLE_STALE_COMMAND on, the command last observed at
   * CYCLE_NAV_RATE_RECOVERY is retained without refresh, so the control clock
   * must jump past the guidance command timeout for that retained command to
   * read as stale. The timeout is derived from the guidance rate and differs
   * between the flight and lockstep builds, so this jump is taken from the same
   * constant rather than a fixed millisecond literal; one extra control period
   * keeps the boundary strictly exceeded. */
  return base_ns + RDD2_GUIDANCE_COMMAND_TIMEOUT_NS + RDD2_CONTROL_PERIOD_NS;
}
