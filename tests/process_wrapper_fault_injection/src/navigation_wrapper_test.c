...
static void navigation_fill_valid_estimate(NavigationEstimatorState *self) {
  ...
  /* reseeded is a per-tick level the block clears every step; mirror that so a
   * re-seed reported on one cycle does not carry into the next. */
  self->status_reseeded = false;
}
...
void NavigationEstimator_dostep(NavigationEstimatorState *self) {
  ...
  case NAV_CYCLE_RECOVERY_MISCONFIGURED:
    ...
    break;
  case NAV_CYCLE_RESEED:
    /* The automatic recovery ladder re-seeded the state this tick; the wrapper
     * must advance the published reset counter once for it. */
    self->status_reseeded = true;
    break;
  default:
    break;
  }
  ...
  observation->output_reseeded = self->status_reseeded;
...
ZTEST(process_wrapper_fault_injection,
  ...
  /* Timestamps handed to the block are filter-relative: the first sample seen
   * captures the shared 64-bit epoch, so the first cycle reads as 0.0 s rather
   * than its absolute wire time. */
  zexpect_within(
      navigation_generated[NAV_CYCLE_NONFINITE_IMU_RESET].imu_timestamp_s, 0.0f,
      1.0e-6f);
  ...
  zexpect_equal(navigation_publications[NAV_CYCLE_RECOVERY_NOMINAL]
                ...
                0U);
  /* A reported re-seed advances the published reset counter exactly once, and
   * a following tick without a re-seed leaves it where it was. */
  zexpect_true(navigation_generated[NAV_CYCLE_RESEED].output_reseeded);
  zexpect_equal(
      navigation_publications[NAV_CYCLE_RESEED].odometry.reset_counter, 1U);
  zexpect_false(navigation_generated[NAV_CYCLE_RESEED_STABLE].output_reseeded);
  zexpect_equal(
      navigation_publications[NAV_CYCLE_RESEED_STABLE].odometry.reset_counter,
      1U);
}
