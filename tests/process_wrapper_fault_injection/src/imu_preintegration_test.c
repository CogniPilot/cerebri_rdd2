...
/* Integrate a run of gap_samples nominal intervals sample by sample, the way
 * an unbroken 800 Hz stream arrives. */
static void run_unbroken_stream(struct rdd2_imu_packet *packet, int gap_samples,
                                bool first_order_hold) {
  struct rdd2_imu_preintegrator state;
  uint64_t timestamp_ns = UINT64_C(2000000000);
  const uint64_t step_ns = (uint64_t)(SAMPLE_DT_S * 1.0e9 + 0.5);
  ...
  (void)rdd2_imu_preintegrator_accumulate(&state, g_gyro, g_accel,
                                          timestamp_ns);
  for (int sample = 0; sample < gap_samples; ++sample) {
    timestamp_ns += step_ns;
    (void)rdd2_imu_preintegrator_accumulate(&state, g_gyro, g_accel,
                                            timestamp_ns);
  }
  rdd2_imu_preintegrator_close(&state, packet, timestamp_ns, g_zero, g_zero);
}

/* Deliver the same span as a single sample after a gap of gap_samples nominal
 * periods, so the interval is bridged rather than integrated directly. */
static void run_bridged_gap(struct rdd2_imu_preintegrator *state,
                            struct rdd2_imu_packet *packet, int gap_samples,
                            bool first_order_hold) {
  uint64_t timestamp_ns = UINT64_C(2000000000);
  const uint64_t step_ns = (uint64_t)(SAMPLE_DT_S * 1.0e9 + 0.5);
  ...
  state->first_order_hold = first_order_hold;
  rdd2_imu_preintegrator_reset(state, g_zero, g_zero);
  (void)rdd2_imu_preintegrator_accumulate(state, g_gyro, g_accel, timestamp_ns);
  /* The nominal period sizes the reconstructed sub-intervals; setting it after
   * the priming sample keeps that sample history-only. */
  state->nominal_sample_period_s = (float)SAMPLE_DT_S;
  timestamp_ns += (uint64_t)gap_samples * step_ns;
  (void)rdd2_imu_preintegrator_accumulate(state, g_gyro, g_accel, timestamp_ns);
  rdd2_imu_preintegrator_close(state, packet, timestamp_ns, g_zero, g_zero);
}

ZTEST(process_wrapper_fault_injection,
      test_a_bridged_gap_matches_the_unbroken_stream_at_constant_rate) {
  const int gap_samples = 80; /* 100 ms at 800 Hz */
  struct rdd2_imu_preintegrator bridged_state;
  struct rdd2_imu_packet unbroken;
  struct rdd2_imu_packet bridged;

  run_unbroken_stream(&unbroken, gap_samples, false);
  run_bridged_gap(&bridged_state, &bridged, gap_samples, false);

  zexpect_true(bridged.valid);
  zexpect_within(bridged.integration_time_s, unbroken.integration_time_s,
                 1.0e-5f);
  for (int axis = 0; axis < 3; ++axis) {
    zexpect_within(bridged.delta_angle_rad[axis], unbroken.delta_angle_rad[axis],
                   1.0e-5f);
    zexpect_within(bridged.delta_velocity_m_s[axis],
                   unbroken.delta_velocity_m_s[axis], 1.0e-4f);
    zexpect_within(bridged.delta_position_m[axis], unbroken.delta_position_m[axis],
                   1.0e-4f);
  }
}

ZTEST(process_wrapper_fault_injection,
      test_a_bridged_gap_matches_the_unbroken_stream_under_first_order_hold) {
  const int gap_samples = 80;
  struct rdd2_imu_preintegrator bridged_state;
  struct rdd2_imu_packet unbroken;
  struct rdd2_imu_packet bridged;

  run_unbroken_stream(&unbroken, gap_samples, true);
  run_bridged_gap(&bridged_state, &bridged, gap_samples, true);
...
ZTEST(process_wrapper_fault_injection,
      test_a_gap_beyond_the_bridge_bound_is_rejected) {
  struct rdd2_imu_preintegrator state;
  struct rdd2_imu_packet packet;
  uint64_t timestamp_ns = UINT64_C(3000000000);
  ...
  (void)rdd2_imu_preintegrator_accumulate(&state, g_gyro, g_accel,
                                          timestamp_ns);
  state.nominal_sample_period_s = (float)SAMPLE_DT_S;
  /* 300 ms exceeds RDD2_IMU_PREINTEGRATION_MAX_BRIDGE_DT_S. */
  timestamp_ns += (uint64_t)(0.3 * 1.0e9);
  {
    bool integrated = rdd2_imu_preintegrator_accumulate(&state, g_gyro, g_accel,
                                                        timestamp_ns);
    zexpect_false(integrated);
  }
  rdd2_imu_preintegrator_close(&state, &packet, timestamp_ns, g_zero, g_zero);

  zexpect_false(packet.valid);
  zexpect_equal(state.rejected_sample_count, 1U);
  zexpect_equal(state.bridged_sample_count, 0U);
  zexpect_equal(packet.sample_count, 0U);
}

ZTEST(process_wrapper_fault_injection, test_bridge_counters_behave) {
  struct rdd2_imu_preintegrator state;
  struct rdd2_imu_packet packet;
  uint64_t timestamp_ns = UINT64_C(4000000000);
  const uint64_t step_ns = (uint64_t)(SAMPLE_DT_S * 1.0e9 + 0.5);
  ...
  /* One normal interval, then one bridged gap, then another normal interval. */
  timestamp_ns += step_ns;
  (void)rdd2_imu_preintegrator_accumulate(&state, g_gyro, g_accel,
                                          timestamp_ns);
  timestamp_ns += (uint64_t)(0.1 * 1.0e9); /* 100 ms bridge */
  (void)rdd2_imu_preintegrator_accumulate(&state, g_gyro, g_accel,
                                          timestamp_ns);
  timestamp_ns += step_ns;
  (void)rdd2_imu_preintegrator_accumulate(&state, g_gyro, g_accel,
  ...
  zexpect_equal(state.bridged_sample_count, 1U);
  zexpect_equal(state.rejected_sample_count, 0U);
  /* Two normal samples plus the one bridging sample each advance the count. */
  zexpect_equal(state.sample_count, 3U);

  rdd2_imu_preintegrator_close(&state, &packet, timestamp_ns, g_zero, g_zero);
  zexpect_true(packet.valid);
  /* The window spans two nominal intervals plus the 100 ms bridge. */
  zexpect_within(packet.integration_time_s,
                 (float)(2.0 * SAMPLE_DT_S + 0.1), 1.0e-5f);
  /* Restart clears the window counters but preserves the fault tallies. */
  zexpect_equal(state.bridged_sample_count, 1U);
}
