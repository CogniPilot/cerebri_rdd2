 ...
 * squared rotation angle exactly as LieGroups.SE23.Quat.exp_mixed does. The
 * third coefficient is only needed by the position increment; callers that do
 * not use it, such as the right Jacobian, pass NULL to skip evaluating it. */
...
static void right_jacobian(float result[3][3], const float rotation[3]) {
  ...
  rotation_series(theta_squared, &first, &second, NULL);
...
/*
 * Compose one interval of duration dt into the running preintegral from a raw
 * start and end sample. Under the first-order hold the interval is the
 * trapezoid-plus-coning form over the two endpoints; under the zero-order
 * hold it holds the end sample across the interval, matching the model's
 * flight default. This is the single per-interval step every accepted
 * sample and every bridged sub-interval passes through, so the packet stays
 * one consistent preintegral regardless of how the interval was formed. It
 * updates the nominal state, the Jacobians, and integration_time_s, but not
 * the sample counters, which the caller owns.
 */
static void integrate_interval(struct rdd2_imu_preintegrator *state,
                               const float start_gyroscope_rad_s[3],
                               const float start_accelerometer_m_s2[3],
                               const float end_gyroscope_rad_s[3],
                               const float end_accelerometer_m_s2[3], float dt,
  ...
  if (first_order_hold) {
    ...
    for (size_t axis = 0U; axis < 3U; ++axis) {
      start_angular_velocity[axis] =
          start_gyroscope_rad_s[axis] -
          state->gyroscope_bias_linearization_rad_s[axis];
      start_specific_force[axis] =
          start_accelerometer_m_s2[axis] -
          state->accelerometer_bias_linearization_m_s2[axis];
      /* Sample differences: the constant bias anchor cancels, so these never
       * subtract two nearly parallel corrected samples. */
      angular_velocity_delta[axis] =
          end_gyroscope_rad_s[axis] - start_gyroscope_rad_s[axis];
      specific_force_delta[axis] =
          end_accelerometer_m_s2[axis] - start_accelerometer_m_s2[axis];
    }
    /* Trapezoid plus the truncated Magnus bracket: the first term is exactly
     * the PX4/ArduPilot trapezoidal increment and the second is the classical
     * coning correction. */
    ...
    for (size_t axis = 0U; axis < 3U; ++axis) {
      rotation_increment[axis] =
          (start_angular_velocity[axis] + 0.5f * angular_velocity_delta[axis]) *
              dt +
          (dt * dt / 12.0f) * bracket[axis];
      velocity_increment[axis] =
          (start_specific_force[axis] + 0.5f * specific_force_delta[axis]) *
              dt +
          (dt * dt / 12.0f) * (sculling_first[axis] - sculling_second[axis]);
      position_increment[axis] = -(dt * dt / 12.0f) * specific_force_delta[axis];
      midpoint_specific_force[axis] =
          start_specific_force[axis] + 0.5f * specific_force_delta[axis];
    }
  } else {
    for (size_t axis = 0U; axis < 3U; ++axis) {
      float corrected_angular_velocity =
          end_gyroscope_rad_s[axis] -
          state->gyroscope_bias_linearization_rad_s[axis];
      float corrected_specific_force =
          end_accelerometer_m_s2[axis] -
          state->accelerometer_bias_linearization_m_s2[axis];

      angular_velocity_delta[axis] = 0.0f;
      specific_force_delta[axis] = 0.0f;
      rotation_increment[axis] = corrected_angular_velocity * dt;
      velocity_increment[axis] = corrected_specific_force * dt;
      position_increment[axis] = 0.0f;
      midpoint_specific_force[axis] = corrected_specific_force;
    }
  }

  quaternion_to_rotation(previous_rotation, state->delta_quaternion);
  quaternion_exp(increment_quaternion, rotation_increment);
  /* The Jacobians read the pre-update preintegral, so compose them before the
   * nominal state advances. */
...
/*
 * Bridge a dropout by integrating the gap as sub_count nominal-period
 * sub-intervals whose rates are linearly interpolated between the previous
 * and the current sample. On a constant-rate stream this reproduces the
 * unbroken sample-by-sample composition exactly; on a varying stream it is
 * the best-available first-order reconstruction of the lost samples. Every
 * sub-interval goes through integrate_interval, so the packet remains one
 * valid preintegral.
 */
static void bridge_interval(struct rdd2_imu_preintegrator *state,
                            const float end_gyroscope_rad_s[3],
                            const float end_accelerometer_m_s2[3], float dt,
                            uint32_t sub_count) {
  const float sub_dt = dt / (float)sub_count;
  bool first_order_hold = state->first_order_hold;

  for (uint32_t sub = 0U; sub < sub_count; ++sub) {
    float fraction_start = (float)sub / (float)sub_count;
    float fraction_end = (float)(sub + 1U) / (float)sub_count;
    float sub_start_gyroscope[3];
    float sub_start_accelerometer[3];
    float sub_end_gyroscope[3];
    float sub_end_accelerometer[3];

    for (size_t axis = 0U; axis < 3U; ++axis) {
      float gyroscope_span =
          end_gyroscope_rad_s[axis] - state->previous_gyroscope_rad_s[axis];
      float accelerometer_span = end_accelerometer_m_s2[axis] -
                                 state->previous_accelerometer_m_s2[axis];

      sub_start_gyroscope[axis] =
          state->previous_gyroscope_rad_s[axis] + fraction_start * gyroscope_span;
      sub_start_accelerometer[axis] =
          state->previous_accelerometer_m_s2[axis] +
          fraction_start * accelerometer_span;
      sub_end_gyroscope[axis] =
          state->previous_gyroscope_rad_s[axis] + fraction_end * gyroscope_span;
      sub_end_accelerometer[axis] =
          state->previous_accelerometer_m_s2[axis] +
          fraction_end * accelerometer_span;
    }
    integrate_interval(state, sub_start_gyroscope, sub_start_accelerometer,
                       sub_end_gyroscope, sub_end_accelerometer, sub_dt,
                       first_order_hold);
  }
}
...
void rdd2_imu_preintegrator_reset(struct rdd2_imu_preintegrator *state,
  ...
  state->bridged_sample_count = 0U;
  /* first_order_hold and nominal_sample_period_s are caller-owned
   * configuration, set once around construction and deliberately preserved
   * across this reset and every restart: a window close must not silently
   * disable the first-order hold or the reset-fallback period. The struct must
   * therefore be zero-initialized before the caller configures those fields.
   * This guards against a struct that was never zeroed reaching the integrator
   * with a garbage fallback period. */
  assert(isfinite(state->nominal_sample_period_s) &&
         state->nominal_sample_period_s >= 0.0f);
}
...
bool rdd2_imu_preintegrator_accumulate(struct rdd2_imu_preintegrator *state,
  ...
  if (measured_interval) {
    if (dt >= RDD2_IMU_PREINTEGRATION_MIN_DT_S &&
      ...
      /* Normal sample. The first-order hold needs the previous sample as the
       * start of the interval; the nominal-period fallback for the first
       * sample after a reset has no start, so it holds the current sample. */
      bool first_order_hold =
          state->first_order_hold && state->previous_sample_valid;

      integrate_interval(state, state->previous_gyroscope_rad_s,
                         state->previous_accelerometer_m_s2, gyroscope_rad_s,
                         accelerometer_m_s2, dt, first_order_hold);
      state->sample_count++;
      integrated = true;
    } else if (dt <= RDD2_IMU_PREINTEGRATION_MAX_BRIDGE_DT_S &&
               state->previous_sample_valid) {
      /* Plausible dropout: reconstruct the lost samples as nominal-period
       * sub-intervals rather than discarding the motion across the gap. */
      float sub_period = state->nominal_sample_period_s > 0.0f
                             ? state->nominal_sample_period_s
                             : RDD2_IMU_PREINTEGRATION_MAX_DT_S;
      uint32_t sub_count = (uint32_t)(dt / sub_period + 0.5f);

      if (sub_count < 1U) {
        sub_count = 1U;
      }
      /* Guarantee every reconstructed sub-interval stays inside the band the
       * closed form is tuned for. */
      while (dt / (float)sub_count > RDD2_IMU_PREINTEGRATION_MAX_DT_S) {
        sub_count++;
      }
      bridge_interval(state, gyroscope_rad_s, accelerometer_m_s2, dt,
                      sub_count);
      state->sample_count++;
      state->bridged_sample_count++;
      integrated = true;
    } else {
