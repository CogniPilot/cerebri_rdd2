...
/* True when the assembled configuration is internally consistent (positive
 * limits, worst-case stddev at least the best-case). The step function already
 * rejects samples under an invalid config; this lets the process log the fault
 * once at start rather than only failing silently at runtime. */
