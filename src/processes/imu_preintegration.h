...
/* Bounds on the measured sample interval. An interval in
 * [MIN_DT_S, MAX_DT_S] is a normal sample and is composed directly.
 *
 * An interval in (MAX_DT_S, MAX_BRIDGE_DT_S] is a plausible logger or
 * transport dropout: samples were lost but the endpoints still bound real
 * motion, so it is bridged rather than discarded. The gap is integrated as a
 * sequence of nominal-period sub-intervals whose rates are linearly
 * interpolated between the previous and the current sample, reusing the same
 * per-interval composition as a normal sample so the packet stays a valid
 * preintegral with consistent Jacobians and integration_time_s. Both
 * ArduPilot (dt clipped to 50 ms) and PX4 (20 ms) integrate the oversized
 * sample rather than drop it; bridging recovers the same motion while keeping
 * each composed sub-interval inside the band the closed form was tuned for.
 *
 * An interval below MIN_DT_S or above MAX_BRIDGE_DT_S is a transport fault,
 * not a measurement, so it advances the sample history without contributing
 * an interval. */
#define RDD2_IMU_PREINTEGRATION_MIN_DT_S 1.0e-5f
#define RDD2_IMU_PREINTEGRATION_MAX_DT_S 0.02f
#define RDD2_IMU_PREINTEGRATION_MAX_BRIDGE_DT_S 0.25f
...
struct rdd2_imu_preintegrator {
  ...
  /* Number of arriving samples whose interval fell in the bridge band and was
   * reconstructed from interpolated sub-intervals instead of being rejected. */
  uint32_t bridged_sample_count;
