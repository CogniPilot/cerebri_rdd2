...
static void motor_output_publish(const rdd2_motor_values_t *motors, const rdd2_motor_raw_t *raw,
...
int rdd2_motor_output_init(void)
{
	...
	motor_output_publish(
		&(rdd2_motor_values_t){0},
		&(rdd2_motor_raw_t){
			.value = {DSHOT_DISARMED, DSHOT_DISARMED, DSHOT_DISARMED, DSHOT_DISARMED},
		},
		false, false);
	return rc;
}
...
#if defined(CONFIG_RDD2_DSHOT) && !defined(CONFIG_RDD2_LOCKSTEP)
		nxp_flexio_dshot_data_set(DEVICE_DT_GET(MOTOR_NODE), i, raw_values[i], false);
#endif
	}
#else
	for (size_t i = 0; i < 4U; i++) {
		...
		pwm_set(DEVICE_DT_GET(MOTOR_NODE), i, PWM_HZ(PWM_FREQ),
			PWM_HZ(PWM_FREQ) / UINT16_MAX * applied_values[i], PWM_POLARITY_NORMAL);
	}
#endif
