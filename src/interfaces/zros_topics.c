...
ZROS_TOPIC_DEFINE_SINGLE_PUBLISHER(rate_command,
                                   synapse_topic_RateCommandData_t);

/* RDD2 owns its synapse_fbs 0.9 topic contract. CSyn resolves these compact
...
static void format_synapse_topic(const struct shell *sh,
  ...
  columns =
      MAX(1U, terminal_width / (TOPIC_SHELL_CELL_WIDTH + TOPIC_SHELL_CELL_GAP));
...
bool rdd2_topic_gnss_copy(synapse_topic_GnssFixData_t *fix,
                          uint32_t *generation) {
