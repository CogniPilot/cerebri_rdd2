{
  ...
  inputs = {
    ...
    rumoca.url = "github:CogniPilot/rumoca/e0c76ac8c2d9865041853804be5d754dc17005bd";
  };
      ...
      packages = forAllSystems (
        ...
        let
          ...
          commonScript = ''
                        ...
                        rdd2_prepare_synapse_fbs() {
                          local app="$1"
                          local package_dir

                          if [ -n "''${RDD2_SYNAPSE_FBS_ROOT:-}" ] &&
                             [ -f "$RDD2_SYNAPSE_FBS_ROOT/zephyr/module.yml" ]; then
                            export RDD2_SYNAPSE_FBS_ROOT
                            return 0
                          fi

                          local build_dir="''${RDD2_SYNAPSE_FBS_BUILD_DIR:-$app/.devenv/state/synapse_fbs-build}"
                          package_dir="$build_dir/synapse_fbs-c"
                          if [ ! -f "$package_dir/zephyr/module.yml" ]; then
                            printf '[deps] provisioning the pinned synapse_fbs C package\n'
                            "$app/scripts/synapse_fbs_package" "$build_dir" >/dev/null
                          fi
                          if [ ! -f "$package_dir/zephyr/module.yml" ]; then
                            printf 'error: synapse_fbs provisioning did not produce %s\n' \
                              "$package_dir" >&2
                            return 1
                          fi
                          export RDD2_SYNAPSE_FBS_ROOT="$package_dir"
                        }
                        ...
                        rdd2_prepare_fmi_plant() {
                          ...
                          # The plant is exported by the FMI backend the Modelica
                          # repository pins for it, which is a separate compiler from the
                          # firmware eFMI generator named by RDD2_RUMOCA_EXECUTABLE. Route
                          # the plant through the repository's own export tool so the
                          # firmware controller pin never selects the plant's FMI kernel.
                          #
                          # The eskf-automatic-reseed model revision bumped the Modelica
                          # repository's own rumoca input to a release whose fmi3 target
                          # rejects the plant's event and condition partitions, so its
                          # default backend can no longer export Vehicles.Rdd2.Plant. The
                          # reseed status fields the firmware needs are generated by the
                          # firmware rumoca and are independent of the plant backend, so
                          # the plant export is pinned back to the last rumoca revision
                          # that exports it, keeping the plant decoupled from the model
                          # revision's backend bump.
                          printf '[deps] exporting the RDD2 FMI 3 plant via the pinned Modelica FMI backend\n'
                          MODELICA_MODELS_ROOT="$RDD2_MODELICA_MODELS_ROOT" \
                            ${pkgs.nix}/bin/nix run "$RDD2_MODELICA_MODELS_ROOT#default" \
                              --override-input rumoca "github:CogniPilot/rumoca/1ae998516e12cb31675c5c14afcd803baa2b7f4a" \
                              -- export rdd2-plant

                          # The exporter writes an unpacked source FMU whose
                          # modelDescription.xml and sources live under a
                          # model-named subdirectory of the output root.
                          plant_root="$RDD2_MODELICA_MODELS_ROOT/artifacts/vehicles/rdd2/plant"
                          description="''${RDD2_RUMOCA_PLANT_DESCRIPTION:-}"
                          if [ -z "$description" ] || [ ! -f "$description" ]; then
                            description="$(find "$plant_root" -name modelDescription.xml -print 2>/dev/null | head -n 1)"
                          fi
                          if [ -z "$description" ] || [ ! -f "$description" ]; then
                            printf 'error: plant export did not produce a modelDescription.xml under %s\n' "$plant_root" >&2
                            return 1
                          fi
                          fmu_dir="$(dirname "$description")"
            ...
            PY
                          ...
                          source="$(find "$fmu_dir/sources" -name '*.c' -print 2>/dev/null | head -n 1)"
                          if [ -z "$source" ] || [ ! -f "$source" ]; then
                            printf 'error: plant export produced no C source under %s\n' "$fmu_dir/sources" >&2
                            return 1
                          fi
                          binary_dir="$fmu_dir/binaries/${system}"
          ...
          rdd2-fastdyn-setup = mkWestApp "rdd2-fastdyn-setup" ''
            ...
            export PKG_CONFIG_PATH="${
              ...
              # Some libraries (for example zlib) install their pkg-config file
              # under share/pkgconfig rather than lib/pkgconfig.
              lib.makeSearchPathOutput "dev" "share/pkgconfig" (mkFastDynCiLibraries pkgs)
            }:$cjson_prefix/lib/pkgconfig:$cjson_prefix/lib64/pkgconfig''${PKG_CONFIG_PATH:+:$PKG_CONFIG_PATH}"
            # QEMU's meson build links against the CPython shared library that
            # matches the build interpreter, so its directory must be on the
            # linker search path even though the interpreter itself is on PATH.
            nix_library_path="${lib.makeLibraryPath (mkFastDynCiLibraries pkgs)}:${lib.getLib pkgs.python3}/lib"
            ...
            # Building QEMU outside a Nix derivation means the compiler wrapper
            # is not handed these library headers automatically, so expose their
            # include directories on the C preprocessor search path. QEMU's
            # meson build only forwards a subset via pkg-config to each target.
            export CPATH="${
              lib.makeSearchPathOutput "dev" "include" (mkFastDynCiLibraries pkgs)
            }:$cjson_prefix/include''${CPATH:+:$CPATH}"

            # Pin the patched QEMU base to the commit the FastDyn icount plugin
            # patch applies against. The upstream default "fastdyn" branch moves
            # (later x86 and RISC-V ports touch plugins/api.c), which makes the
            # patch fail to apply; this commit is the expected patched base.
            "$FASTDYN_ROOT/setup.sh" \
              --venv "$FASTDYN_STATE/venv" \
              --build-qemu \
              --qemu-root "$FASTDYN_STATE/qemu" \
              --qemu-ref 404b469f1f3b248e035e46cbfcf49f36f46ae793 \
              --with-rumoca \
              --skip-optifuzz
